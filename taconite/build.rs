...
//!   `DEP_TACONITE_XRT_DYNLIST` for a dependent's build script to re-emit as
//!   `-Wl,--dynamic-list=…`.
//!
//! - **dynamic** (`$XRT_ROOT`, default `/opt/xilinx/xrt`, the tree XRT's
//!   `setup.sh` puts on the loader path): links `libxrt_coreutil.so.2`,
//!   publishes the lib dir as `DEP_TACONITE_XRT_LIBDIR` for dependents to re-emit
//!   as an rpath.
//!
...
fn main() {
    ...
    let archive = out_dir.join("libtaconite_xrt.a");
    ...
    println!("cargo:rustc-link-lib=static=taconite_xrt");
    ...
    if let Some(root) = &static_root {
        ...
        // location is not watched — `cargo clean -p taconite` after creating
        // it.
        ...
        // tests); dependents re-emit it from DEP_TACONITE_XRT_DYNLIST.
        ...
        // is); dependents re-emit it from DEP_TACONITE_XRT_LIBDIR, or rely on
