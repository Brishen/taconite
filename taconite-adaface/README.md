...
fn main() -> Result<(), taconite_adaface::Error> {
    ...
    let _ = taconite_adaface::cosine(&embedding, &embedding);
    Ok(())
}
```
...
- With XRT, the link flags propagate to your binary automatically. The
  **rpath does not** (cargo drops `rustc-link-arg` across crates): either
  ...
  `links = "taconite_adaface"` key (this is what the `run_ir*` binaries do):

  ```rust
  // build.rs of your app
  fn main() {
      println!("cargo:rustc-link-arg=-Wl,-rpath,$ORIGIN/lib");
      if let Ok(lib) = std::env::var("DEP_TACONITE_ADAFACE_LIBDIR") {
