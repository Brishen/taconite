...
pub use iron_bundle::{DType, Entry, Store, Xclbin};
...
impl Manifest {
    pub fn load(dir: &Path) -> Result<Self, Error> {
        let base = iron_bundle::Manifest::load(dir, VERSION)?;
        let mut m = Manifest {
            dir: dir.to_path_buf(),
            base: base.clone(),
            gemms: HashMap::new(),
        ...
        let name = |r: &iron_bundle::Record| r.str("name").unwrap_or("MLIR_AIE").to_string();
