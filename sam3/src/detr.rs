        ...
        par_rows(&mut pb, nh * l, |r0, piece| {
            ...
            for (ri, row) in piece.chunks_mut(nh * l).enumerate() {
                ...
                for hh in 0..nh {
                    ...
                    for j in 0..l {
                        row[hh * l + j] = iron_xrt::f32_to_bf16(row_f[j]);
                    }
                }
            }
        });
        ...
        for i in 0..c.d_layers {
            ...
            par_rows(&mut ab, 2 * d, |r0, piece| {
                ...
                for (ri, row) in piece.chunks_mut(2 * d).enumerate() {
                    ...
                    for j in 0..d {
                        row[j] = iron_xrt::f32_to_bf16(h[j] + pos[r * d + j]);
                        row[d + j] = iron_xrt::f32_to_bf16(h[j]);
                    }
                }
            });
        ...
        // every layer's vision keys (from enc + pos) and values (from enc)
        {
            ...
            par_rows(&mut ab, 2 * d, |r0, piece| {
                for (ri, row) in piece.chunks_mut(2 * d).enumerate() {
                    let r = r0 + ri;
                    for j in 0..d {
                        row[j] = iron_xrt::f32_to_bf16(enc[r * d + j] + pos[r * d + j]);
                        row[d + j] = iron_xrt::f32_to_bf16(enc[r * d + j]);
                    }
                }
            });
