...
use sam3::{Instance, Output, Sam3, Text, instances, preprocess};
...
fn check(dir: &Path) -> R<bool> {
    ...
    println!("host pieces:");
    {
        ...
        let packed = sam3::pack::pack_b(spec, st.f32("pack_test.B")?, Some(st.f32("pack_test.bias")?));
    ...
    let conf: Vec<usize> = (0..q).filter(|&i| sam3::cpu::sigmoid(rl[i]) > 0.1).collect();
...
fn segment(args: &[String]) -> R<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--threads" => sam3::cpu::set_threads(args[i + 1].parse()?),
...
fn bench_copy() -> R<()> {
    ...
    for _ in 0..3 {
        let t = Instant::now();
        sam3::npu::push(&src, &mut b)?;
        let push_s = t.elapsed().as_secs_f64();
        let t = Instant::now();
        let v = sam3::npu::pull(&b, n)?;
