...
fn main() {
    ...
    let archive = Path::new(&out_dir).join("libironxrt.a");
    ...
    // binaries re-emit the rpath from DEP_IRONXRT_LIBDIR (published via the
    // `links = "ironxrt"` key and the libdir line below) or rely on XRT's
