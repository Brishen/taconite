...
// resident (when they all fit within IRON_XRT_CTX_CACHE) means a repeated
...
struct iron_xrt_session {
    ...
    // Instrumentation (printed on close when IRON_XRT_TIMING is set).
...
extern "C" iron_xrt_session *iron_xrt_open(int device_index, char *err,
    ...
    try {
        ...
        if (const char *env = std::getenv("IRON_XRT_CTX_CACHE")) {
...
extern "C" void iron_xrt_close(iron_xrt_session *s) {
    if (s && std::getenv("IRON_XRT_TIMING")) {
        std::fprintf(stderr,
